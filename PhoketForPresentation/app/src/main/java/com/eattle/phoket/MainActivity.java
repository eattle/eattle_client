...
public class MainActivity extends ActionBarActivity implements ActionBar.TabListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toUSB = (Button)findViewById(R.id.toUSB);
        ...
        usbDeviceHost.start(this,paramBlock);
    ...
    protected void onResume() {
        ...
        if(CONSTANT.ISUSBCONNECTED == 1)
            toUSB.setVisibility(View.VISIBLE);//하단에 USB 버튼을 일단 없앤다
        else
            toUSB.setVisibility(View.GONE);//하단에 USB 버튼을 일단 없앤다
...
/*        if (CONSTANT.PASSWORD == 0) {//비밀 번호 해제 안됬으면
    ...
    public static BlockDeviceApp paramBlock =  new BlockDeviceApp() {
        @Override
        public void onConnected(BlockDevice originalBlockDevice) {
            Log.d("usb","onConnected 호출");
            ...
            AlbumFullActivity.fileSystem.incaseSearchTable(blockDevice);
            ...
            //USB에 있는 모든 파일들을 다 지운다
            //for(int i=0;i<=fileSystem.endpoint;i++)
            //    fileSystem.delete(fileSystem.searchtable[i][0],CONSTANT.BLOCKDEVICE);
...
/*
    ...
    public void onClick(View v){
        switch(v.getId()){
            case R.id.toUSB:
                Intent intent = new Intent(getApplicationContext(), USBMainActivity.class);
                startActivity(intent);
        }
    }
...
//        d.setView(r);
    ...
    private byte[] getDBFromUSB(String outString, CachedBlockDevice blockDevice) {//내보내기
        ...
        if (result[0] == -1) {
            Log.d(Tag,"[getDBFromUSB]USB에 Phoket DB가 존재하지 않습니다");

            //Toast.makeText(this, "값이 잘못들어왔습니다", Toast.LENGTH_SHORT).show();
            return null;
        } else {
            //USB에서 import 할 DB가 존재하기 때문에
            //스마트폰에 있는 기존의 APP DB를 삭제한다
